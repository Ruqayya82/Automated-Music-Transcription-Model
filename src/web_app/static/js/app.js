...
async function transcribeAudio() {
    ...
    try {
        ...
        setProgress(60);
        progressText.textContent = 'Detecting musical notes from audio...';
        
        const data = await response.json();
        
        if (data.success) {
        setProgress(80);
        progressText.textContent = 'Creating MIDI file from detected notes...';
...
function showResults(data) {
    ...
    const method = data.metadata.method === 'model' ? 'ML Model' : 'Signal Processing';
    const processingTime = data.metadata.processing_time || 'N/A';
    methodEl.textContent = `${method} (${processingTime}s)`;
